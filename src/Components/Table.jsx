...
const Table = ({ _id, unit }) => {
    ...
    const fetchCropData = async () => {
        try {
            const res = await fetch(`http://localhost:3000/corps/${_id}`)
            if (!res.ok) {
    ...
    const handleAction = async (interestId, newStatus) => {
        const confirmAction = window.confirm(
            `Are you sure you want to ${newStatus} this interest?`
        );
        ...
        try {
            ...
            const res = await fetch(
                `http://localhost:3000/crops/${_id}/interests/${interestId}`,
                {
                    method: "PATCH",
  ...
  {crop.interests.map((interest) => (
    ...
    >
      ...
      <p className="text-gray-800">
        <span className="font-medium">Quantity:</span> {interest.quantity}{unit}
      </p>
      ...
      <table className="min-w-full text-sm text-left text-gray-700">
        ...
        <tbody>
          {crop.interests.map((interest) => (
            ...
            >
              <td className="px-6 py-3 text-[16px] text-amber-600 font-medium">{interest.userName}</td>
              <td className="px-6 text-black text-lg py-3">{interest.quantity}{unit}</td>
              <td className="px-6 text-black text-[16px] py-3">{interest.message}</td>
